...
function resolveRobinhoodApiRange() {
  const endDate = toDateString(new Date());
  const startRaw =
    process.env.ROBINHOOD_API_START_DATE?.trim() ||
    config.SNAPTRADE_START_DATE ||
    config.SNAPTRADE_FETCH_START_DATE ||
    "2025-01-01";
  const start = new Date(startRaw);
  if (Number.isNaN(start.getTime())) {
    throw new Error(`Invalid ROBINHOOD_API_START_DATE: ${startRaw}`);
  }
  return {
...
export async function runImportRobinhoodApi() {
  assertSyncConfig();
  const includeAll = config.SNAPTRADE_INCLUDE_ALL === "1";
  const { startDate, endDate } = resolveRobinhoodApiRange();
  ...
  let robinhoodAccountsProcessed = 0;
  let robinhoodOrdersSeen = 0;

  for (const account of robinhoodAccounts) {
    robinhoodAccountsProcessed += 1;
    ...
    // opening BUY happened before ROBINHOOD_API_START_DATE.
  ...
  return {
    ...
    startDate,
    endDate,
    robinhoodAccounts: robinhoodAccounts.length,
    robinhoodAccountsProcessed,
